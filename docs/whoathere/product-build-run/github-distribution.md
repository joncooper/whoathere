...
- public repo or public release assets for no-auth cloud Mac installs
...
Create the repository and push this local repo:

```sh
gh repo create joncooper/whoathere --public --source=. --remote=origin --push
```
...
Use a private repo only if the test Mac will authenticate with GitHub before downloading the
release. The no-auth `curl` installer path assumes public release assets.
