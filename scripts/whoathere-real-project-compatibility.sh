...
printf 'workspace\tecosystem\tpackage_risk_exit\tverdict\tpackage_count\tscanner_exit\tscanner_clean\telapsed_seconds\toutput\n' > "$SUMMARY"
...
for spec in "$@"; do
  ...
  if [ "$RUN_SCANNERS" = "1" ]; then
    if run_capture "$scanner_output" "$CLI" scanners run --workspace "$workspace" --ecosystem "$ecosystem" --state-dir "$STATE_DIR" --execute --json; then
      scanner_exit=0
    else
      scanner_exit=$?
    fi
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$workspace" "$ecosystem" "$package_status" "$verdict" "$count" "$scanner_exit" "$scanner_clean" "$elapsed" "$package_output" >> "$SUMMARY"
  printf 'real_project_case=%s ecosystem=%s package_risk_exit=%s verdict=%s package_count=%s scanner_exit=%s scanner_clean=%s elapsed_seconds=%s output=%s\n' \
    "$workspace" "$ecosystem" "$package_status" "$verdict" "$count" "$scanner_exit" "$scanner_clean" "$elapsed" "$package_output"
done
