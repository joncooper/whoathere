...
wait_for_server_exit() {
  ...
  for ((attempt = 1; attempt <= SERVER_WAIT_ATTEMPTS; attempt += 1)); do
    if ! kill -0 "$pid" 2>/dev/null; then
      ...
      SERVER_PID=""
      grep -q 'served_requests=3' "$log_path"
...
main() {
  local npm_port npm_log pip_port pip_log

  npm_port="$(pick_port)"
  npm_log="$RUN_DIR/npm-server.log"
  start_server "$npm_port" "$npm_log"
  run_npm_smoke "$npm_port" || {
    print_log "npm-server" "$npm_log"
    return 1
  }
  wait_for_server_exit_and_routes "$npm_log" 'route_kind=registry_npm_metadata' 'route_kind=registry_npm_tarball'

  pip_port="$(pick_port)"
  pip_log="$RUN_DIR/pip-server.log"
  start_server "$pip_port" "$pip_log"
  run_pip_smoke "$pip_port" || {
    print_log "pip-server" "$pip_log"
    return 1
  }
  wait_for_server_exit_and_routes "$pip_log" 'route_kind=registry_pypi_metadata' 'route_kind=registry_pypi_file'
