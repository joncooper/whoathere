...
pub fn decode_linux_vz_network_evidence_payload_v1(
    ...
    let fixture_case = match wire.fixture_case.as_str() {
        ...
        "private_address_connect"
            if wire.network_family == "ipv4"
                && wire.network_source == "10.0.0.2"
                && wire.network_target == "10.0.0.1" =>
        {
            LinuxVzTelemetryConformanceCaseV1::PrivateAddressConnect
        }
        "link_local_connect"
            if wire.network_family == "ipv4"
                && wire.network_source == "169.254.100.2"
                && wire.network_target == "169.254.100.1" =>
        {
        ...
        "public_address_connect"
            if wire.network_family == "ipv4"
                && wire.network_source == "198.51.100.2"
                && wire.network_target == "198.51.100.1" =>
        {
    ...
    let (expected_action, expected_protocol, expected_socket_state, expected_event_kind) =
        ...
        } else if fixture_case == LinuxVzTelemetryConformanceCaseV1::DnsMalformed {
            ("dns_malformed", "udp", "unconnected_bound", "sendto")
        } else if fixture_case == LinuxVzTelemetryConformanceCaseV1::EncryptedDnsConnect {
            ("encrypted_dns_connect", "tcp", "syn_sent", "connect")
    ...
    let expected_target_port = match fixture_case {
        ...
        LinuxVzTelemetryConformanceCaseV1::EncryptedDnsConnect => 853,
        _ => 443,
    };
...
mod tests {
    ...
    #[test]
    fn private_payload_binds_exact_rfc1918_sinkhole() {
        let mut value: serde_json::Value = serde_json::from_slice(&payload()).unwrap();
        value["fixture_case"] = serde_json::json!("private_address_connect");
        value["network_source"] = serde_json::json!("10.0.0.2");
        value["network_target"] = serde_json::json!("10.0.0.1");
        let encoded = serde_json_canonicalizer::to_vec(&value).unwrap();
        let evidence = decode_linux_vz_network_evidence_payload_v1(&encoded).unwrap();
        assert_eq!(
            evidence.fixture_case(),
    ...
    #[test]
    fn link_local_payload_binds_exact_sinkhole() {
        let mut value: serde_json::Value = serde_json::from_slice(&payload()).unwrap();
        value["fixture_case"] = serde_json::json!("link_local_connect");
        value["network_source"] = serde_json::json!("169.254.100.2");
        value["network_target"] = serde_json::json!("169.254.100.1");
        let encoded = serde_json_canonicalizer::to_vec(&value).unwrap();
        let evidence = decode_linux_vz_network_evidence_payload_v1(&encoded).unwrap();
        assert_eq!(
            evidence.fixture_case(),
    ...
    #[test]
    fn public_payload_binds_exact_documentation_sinkhole() {
        let mut value: serde_json::Value = serde_json::from_slice(&payload()).unwrap();
        value["fixture_case"] = serde_json::json!("public_address_connect");
        value["network_source"] = serde_json::json!("198.51.100.2");
        value["network_target"] = serde_json::json!("198.51.100.1");
        let encoded = serde_json_canonicalizer::to_vec(&value).unwrap();
        let evidence = decode_linux_vz_network_evidence_payload_v1(&encoded).unwrap();
        assert_eq!(
            evidence.fixture_case(),
    ...
    #[test]
    fn dns_malformed_payload_binds_exact_sinkhole() {
        let mut value: serde_json::Value = serde_json::from_slice(&payload()).unwrap();
        value["fixture_case"] = serde_json::json!("dns_malformed");
        value["network_action"] = serde_json::json!("dns_malformed");
        value["network_protocol"] = serde_json::json!("udp");
        value["network_socket_state"] = serde_json::json!("unconnected_bound");
        value["network_target"] = serde_json::json!("192.0.2.53");
        value["network_target_port"] = serde_json::json!("53");
    ...
    #[test]
    fn encrypted_dns_payload_binds_exact_dot_sinkhole() {
        let mut value: serde_json::Value = serde_json::from_slice(&payload()).unwrap();
        value["fixture_case"] = serde_json::json!("encrypted_dns_connect");
        value["network_action"] = serde_json::json!("encrypted_dns_connect");
        value["network_target"] = serde_json::json!("192.0.2.53");
        value["network_target_port"] = serde_json::json!("853");
        let encoded = serde_json_canonicalizer::to_vec(&value).unwrap();
        let evidence = decode_linux_vz_network_evidence_payload_v1(&encoded).unwrap();
        assert_eq!(
            evidence.fixture_case(),
