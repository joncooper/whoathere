...
use whoathere_detonation::{
    decode_and_validate_wheel_scenario_template_v1, ValidatedWheelScenarioTemplateWireV1,
    WheelScenarioKindV1, WheelScenarioTemplateV1,
};

pub const MACOS_WHEEL_RUN_SPEC_SCHEMA_V1: &str = "whoathere.macos_wheel_run_spec.v1";
pub const MACOS_WHEEL_RUN_SPEC_CANONICALIZATION_V1: &str = "rfc8785.jcs.v1";
pub const MACOS_WHEEL_GUEST_PROTOCOL_V1: &str = "whoathere.wheel_artifact_scenario.v1";
pub const MAX_MACOS_WHEEL_RUN_SPEC_BYTES_V1: usize = 512 * 1024;
pub const MACOS_WHEEL_PACKAGE_USERNAME_V1: &str = "_whoatherepkg";

#[derive(Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct MacosWheelBackendIdentityV1 {
...
impl fmt::Debug for MacosWheelBackendIdentityV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MacosWheelBackendIdentityV1")
...
impl MacosWheelBackendIdentityV1 {
    ...
    pub fn new(
        ...
        let value = Self {
            ...
            guest_auth_public_key_sha256,
            runner_configuration_sha256,
            package_username: MACOS_WHEEL_PACKAGE_USERNAME_V1.to_string(),
            package_uid,
            package_gid,
            ...
            guest_protocol_sha256: Sha256Digest::from_bytes(
                MACOS_WHEEL_GUEST_PROTOCOL_V1.as_bytes(),
            ),
        };
        value.validate()?;
        Ok(value)
    ...
    fn validate(&self) -> Result<(), MacosArtifactRunErrorV1> {
        if !valid_identity(&self.base_generation_id)
            ...
            || self.package_username != MACOS_WHEEL_PACKAGE_USERNAME_V1
            || self.package_uid == 0
            || self.package_gid == 0
...
pub struct MacosWheelBackendCapabilitiesV1 {
    identity: MacosWheelBackendIdentityV1,
...
impl MacosWheelBackendCapabilitiesV1 {
    pub fn inert_first_slice(identity: MacosWheelBackendIdentityV1) -> Self {
        Self {
            identity,
    ...
    pub fn identity(&self) -> &MacosWheelBackendIdentityV1 {
        &self.identity
    }
}
...
#[serde(deny_unknown_fields)]
struct MacosWheelRunSpecWireV1 {
    schema_version: String,
    canonicalization: String,
    template: serde_json::Value,
    template_sha256: Sha256Digest,
    backend_identity: MacosWheelBackendIdentityV1,
    clone_policy: MacosArtifactClonePolicyV1,
    network_configuration: MacosArtifactNetworkConfigurationV1,
    reuse_policy: MacosArtifactVmReusePolicyV1,
...
pub struct MacosWheelRunSpecV1 {
...
impl fmt::Debug for MacosWheelRunSpecV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MacosWheelRunSpecV1")
...
impl MacosWheelRunSpecV1 {
    ...
    pub fn backend_identity(&self) -> &MacosWheelBackendIdentityV1 {
        &self.backend_identity
    }
}

pub fn compile_macos_wheel_run_spec_v1(
    template: &WheelScenarioTemplateV1,
    backend: &MacosWheelBackendCapabilitiesV1,
) -> Result<MacosWheelRunSpecV1, MacosArtifactRunErrorV1> {
    ...
    let validated = decode_and_validate_wheel_scenario_template_v1(&template_bytes)
        .map_err(|_| MacosArtifactRunErrorV1::InvalidTemplate)?;
    ...
    let wire = MacosWheelRunSpecWireV1 {
        schema_version: MACOS_WHEEL_RUN_SPEC_SCHEMA_V1.to_string(),
        canonicalization: MACOS_WHEEL_RUN_SPEC_CANONICALIZATION_V1.to_string(),
        ...
        guest_protocol: MACOS_WHEEL_GUEST_PROTOCOL_V1.to_string(),
    };
    let canonical_json = serde_json_canonicalizer::to_vec(&wire)
        .map_err(|_| MacosArtifactRunErrorV1::Serialization)?;
    if canonical_json.len() > MAX_MACOS_WHEEL_RUN_SPEC_BYTES_V1 {
        return Err(MacosArtifactRunErrorV1::LimitExceeded);
    }
    Ok(MacosWheelRunSpecV1 {
...
pub fn decode_and_validate_macos_wheel_run_spec_v1(
    bytes: &[u8],
) -> Result<MacosWheelRunSpecV1, MacosArtifactRunErrorV1> {
    if bytes.is_empty() || bytes.len() > MAX_MACOS_WHEEL_RUN_SPEC_BYTES_V1 {
        return Err(MacosArtifactRunErrorV1::LimitExceeded);
    }
    let mut deserializer = serde_json::Deserializer::from_slice(bytes);
    let wire = MacosWheelRunSpecWireV1::deserialize(&mut deserializer)
    ...
    let canonical = serde_json_canonicalizer::to_vec(&wire)
        .map_err(|_| MacosArtifactRunErrorV1::Serialization)?;
    if canonical != bytes
        || wire.schema_version != MACOS_WHEEL_RUN_SPEC_SCHEMA_V1
        || wire.canonicalization != MACOS_WHEEL_RUN_SPEC_CANONICALIZATION_V1
        ...
        || wire.guest_protocol != MACOS_WHEEL_GUEST_PROTOCOL_V1
    {
...
fn require_backend_match(
    template: &ValidatedWheelScenarioTemplateWireV1,
    backend: &MacosWheelBackendIdentityV1,
