...
pub const MACOS_LINUX_VZ_TELEMETRY_HOST_RECEIPT_SCHEMA_V1: &str =
    "whoathere.macos_linux_vz_telemetry_host_receipt.v1";
const HOST_RECEIPT_SIGNATURE_DOMAIN_V1: &[u8] =
    b"whoathere.macos_linux_vz_telemetry_host_receipt.signature.v1\0";

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LinuxVzTelemetryHostObservationClaimsV1 {
...
impl LinuxVzTelemetryHostObservationClaimsV1 {
    ...
    pub fn packet_sensor_healthy(&self) -> bool {
        self.packet_sensor_healthy
    }
    ...
    pub fn guest_channel_terminated(&self) -> bool {
        self.guest_channel_terminated
    }

    pub fn vm_started(&self) -> bool {
        self.vm_started
    }

    pub fn vm_stopped(&self) -> bool {
        self.vm_stopped
    }

    pub fn clone_destroyed(&self) -> bool {
        self.clone_destroyed
    }

    pub fn external_frames_forwarded(&self) -> u64 {
        self.external_frames_forwarded
    }
...
struct UnsignedHostReceiptWireV1 {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VerifiedLinuxVzTelemetryHostReceiptV1 {
    challenge_sha256: Sha256Digest,
    observed_sensors: Vec<ArtifactProtectedTelemetrySensorV1>,
    claims: LinuxVzTelemetryHostObservationClaimsV1,
}

impl VerifiedLinuxVzTelemetryHostReceiptV1 {
    ...
    pub fn claims(&self) -> &LinuxVzTelemetryHostObservationClaimsV1 {
        &self.claims
    }
...
pub fn sign_macos_linux_vz_telemetry_host_receipt_v1(
    ...
    let unsigned = unsigned_host_receipt_v1(challenge, run_spec, claims);
    ...
    let receipt = HostReceiptWireV1::from_unsigned(unsigned, lower_hex_v1(&signature.to_bytes()));
...
pub fn verify_macos_linux_vz_telemetry_host_receipt_v1(
    challenge: &MacosLinuxVzTelemetryConformanceChallengeV1,
    run_spec: &MacosLinuxVzTelemetryConformanceRunSpecV1,
    backend: &UnqualifiedMacosLinuxVzTelemetryBackendIdentityV1,
    receipt_bytes: &[u8],
    verifying_key_bytes: [u8; 32],
    expected_claims: &LinuxVzTelemetryHostObservationClaimsV1,
) -> Result<VerifiedLinuxVzTelemetryHostReceiptV1, MacosLinuxVzTelemetryEvidenceErrorV1> {
    validate_context_v1(challenge, run_spec, backend)?;
    ...
    let receipt = HostReceiptWireV1::deserialize(&mut deserializer)
    ...
    let expected = unsigned_host_receipt_v1(challenge, run_spec, expected_claims);
    ...
    Ok(VerifiedLinuxVzTelemetryHostReceiptV1 {
        challenge_sha256: challenge.challenge_sha256().clone(),
        observed_sensors: run_spec.expected_host_sensors_v1(),
...
fn unsigned_host_receipt_v1(
    challenge: &MacosLinuxVzTelemetryConformanceChallengeV1,
    run_spec: &MacosLinuxVzTelemetryConformanceRunSpecV1,
    claims: &LinuxVzTelemetryHostObservationClaimsV1,
) -> UnsignedHostReceiptWireV1 {
    UnsignedHostReceiptWireV1 {
        schema_version: MACOS_LINUX_VZ_TELEMETRY_HOST_RECEIPT_SCHEMA_V1.to_string(),
        authority: "host_packet_and_vm_lifecycle".to_string(),
        challenge_sha256: challenge.challenge_sha256().clone(),
        ...
        dropped_frame_count: claims.dropped_frame_count.to_string(),
        packet_sensor_healthy: claims.packet_sensor_healthy,
        evidence_truncated: claims.evidence_truncated,
        guest_channel_terminated: claims.guest_channel_terminated,
        vm_started: claims.vm_started,
        vm_stopped: claims.vm_stopped,
        clone_destroyed: claims.clone_destroyed,
        external_frames_forwarded: claims.external_frames_forwarded.to_string(),
...
impl HostReceiptWireV1 {
    fn from_unsigned(value: UnsignedHostReceiptWireV1, signature_ed25519_hex: String) -> Self {
        Self {
            ...
            packet_sensor_healthy: value.packet_sensor_healthy,
            evidence_truncated: value.evidence_truncated,
            guest_channel_terminated: value.guest_channel_terminated,
            vm_started: value.vm_started,
            vm_stopped: value.vm_stopped,
            clone_destroyed: value.clone_destroyed,
            external_frames_forwarded: value.external_frames_forwarded,
    ...
    fn into_unsigned(self) -> UnsignedHostReceiptWireV1 {
        UnsignedHostReceiptWireV1 {
            ...
            packet_sensor_healthy: self.packet_sensor_healthy,
            evidence_truncated: self.evidence_truncated,
            guest_channel_terminated: self.guest_channel_terminated,
            vm_started: self.vm_started,
            vm_stopped: self.vm_stopped,
            clone_destroyed: self.clone_destroyed,
            external_frames_forwarded: self.external_frames_forwarded,
...
fn signature_message_v1(challenge: &[u8], unsigned_receipt: &[u8]) -> Vec<u8> {
    let mut message = Vec::with_capacity(
        HOST_RECEIPT_SIGNATURE_DOMAIN_V1.len() + challenge.len() + 1 + unsigned_receipt.len(),
    );
    message.extend_from_slice(HOST_RECEIPT_SIGNATURE_DOMAIN_V1);
