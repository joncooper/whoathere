use crate::{
    decode_macos_wheel_guest_auth_challenge_v1, read_macos_wheel_guest_control_frame_v1,
    sign_macos_wheel_guest_auth_response_v1, sign_macos_wheel_guest_staging_receipt_v1,
    stage_macos_wheel_guest_submission_v1, write_macos_wheel_guest_control_frame_v1,
...
pub enum MacosWheelGuestSupervisorPrimaryErrorV1 {
    InvalidConfiguration,
    Control(MacosWheelGuestControlErrorV1),
    Authentication(MacosWheelGuestAuthErrorV1),
    Staging(MacosWheelGuestStagingErrorV1),
    BindingMismatch,
}

impl MacosWheelGuestSupervisorPrimaryErrorV1 {
    pub const fn reason_code(self) -> &'static str {
        match self {
            Self::InvalidConfiguration => "macos_wheel_guest_supervisor_configuration_invalid",
            Self::Control(error) => error.reason_code(),
            Self::Authentication(error) => error.reason_code(),
            Self::Staging(error) => error.reason_code(),
            Self::BindingMismatch => "macos_wheel_guest_supervisor_binding_mismatch",
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MacosWheelGuestSupervisorFailureV1 {
    primary: MacosWheelGuestSupervisorPrimaryErrorV1,
    staging_cleanup_failed: bool,
}

impl MacosWheelGuestSupervisorFailureV1 {
    fn new(primary: MacosWheelGuestSupervisorPrimaryErrorV1) -> Self {
        Self {
            primary,
    ...
    pub fn primary(&self) -> MacosWheelGuestSupervisorPrimaryErrorV1 {
        self.primary
    }
...
impl fmt::Display for MacosWheelGuestSupervisorFailureV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(self.reason_code())
    }
}

impl std::error::Error for MacosWheelGuestSupervisorFailureV1 {}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MacosWheelGuestNonExecutingSessionObservationV1 {
...
impl MacosWheelGuestNonExecutingSessionObservationV1 {
...
/// Authenticate, stage, rehash, clean, and attest one wheel without executing package code.
pub fn run_macos_wheel_guest_nonexecuting_session_v1<R: Read, W: Write>(
    reader: &mut R,
    writer: &mut W,
    signing_seed: [u8; 32],
    auth_claims: &MacosWheelGuestAuthClaimsV1,
    staging_policy: &MacosWheelGuestStagingPolicyV1,
) -> Result<MacosWheelGuestNonExecutingSessionObservationV1, MacosWheelGuestSupervisorFailureV1> {
    let signing_seed = Zeroizing::new(signing_seed);
    if staging_policy.package_uid() != auth_claims.package_uid() {
        return Err(MacosWheelGuestSupervisorFailureV1::new(
            MacosWheelGuestSupervisorPrimaryErrorV1::InvalidConfiguration,
        ));
    }

    let challenge_body = read_macos_wheel_guest_control_frame_v1(
        reader,
        MacosWheelGuestControlFrameTypeV1::AuthenticationChallenge,
        MAX_MACOS_WHEEL_GUEST_AUTH_BYTES_V1,
    )
    .map_err(|error| {
        MacosWheelGuestSupervisorFailureV1::new(MacosWheelGuestSupervisorPrimaryErrorV1::Control(
            error,
        ))
    })?;
    let challenge =
        decode_macos_wheel_guest_auth_challenge_v1(&challenge_body).map_err(|error| {
            MacosWheelGuestSupervisorFailureV1::new(
                MacosWheelGuestSupervisorPrimaryErrorV1::Authentication(error),
            )
        })?;
    let auth_response =
        sign_macos_wheel_guest_auth_response_v1(&challenge, *signing_seed, auth_claims).map_err(
            |error| {
                MacosWheelGuestSupervisorFailureV1::new(
                    MacosWheelGuestSupervisorPrimaryErrorV1::Authentication(error),
                )
    ...
    let mut staged =
        stage_macos_wheel_guest_submission_v1(reader, staging_policy).map_err(|error| {
            MacosWheelGuestSupervisorFailureV1::new(
                MacosWheelGuestSupervisorPrimaryErrorV1::Staging(error),
            )
        })?;
    let prepared = (|| {
        ...
        {
            return Err(MacosWheelGuestSupervisorPrimaryErrorV1::BindingMismatch);
        }
        let rehash = staged
            .verify_prelaunch()
            .map_err(MacosWheelGuestSupervisorPrimaryErrorV1::Staging)?;
        ...
        .map_err(MacosWheelGuestSupervisorPrimaryErrorV1::Authentication)?;
        let receipt = sign_macos_wheel_guest_staging_receipt_v1(
            &challenge,
            *signing_seed,
            auth_claims,
            &staging_claims,
        ...
        .map_err(MacosWheelGuestSupervisorPrimaryErrorV1::Authentication)?;
        let observation = MacosWheelGuestNonExecutingSessionObservationV1 {
    ...
    let (receipt, observation) = match prepared {
        ...
        Ok(_) => {
            return Err(MacosWheelGuestSupervisorFailureV1 {
                primary: MacosWheelGuestSupervisorPrimaryErrorV1::Staging(
                    MacosWheelGuestStagingErrorV1::CleanupFailed,
                ),
