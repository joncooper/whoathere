...
pub enum MacosLinuxVzPackageExecutionStageV1 {
    ...
    /// Execute the console wrapper generated by the exact wheel installation. The declared
    /// module/callable remains bound data used to validate the wrapper's expected target.
    #[serde(rename = "python_installed_generated_console_wrapper_help_probe")]
    ...
    /// Execute the generated console wrapper in its own fresh environment with an empty argv.
    #[serde(rename = "python_installed_generated_console_wrapper_no_arguments_probe")]
    PythonConsoleEntryPointNoArgumentsProbe {
        command_name: String,
        module: String,
        callable: String,
        target_sha256: Sha256Digest,
    ...
    /// Bind a valid exact empty PEP 517 closure without invoking pip or accepting a payload.
    PythonValidateEmptySdistBuildClosure {
...
impl MacosLinuxVzPackageExecutionStageV1 {
    pub const fn stage_name(&self) -> &'static str {
        match self {
            ...
            Self::PythonConsoleEntryPointNoArgumentsProbe { .. } => {
            ...
            Self::PythonValidateEmptySdistBuildClosure { .. } => {
                "python_validate_empty_sdist_build_closure"
            }
...
fn sdist_stages_v1(
    ...
    if build.build_closure().artifacts().is_empty() {
        stages.push(
...
mod tests {
    ...
    #[test]
    fn console_argument_profiles_have_distinct_closed_probe_stages() {
        let target_sha256 = digest("fixture_pkg.cli:main");
        let profiles = [
            (
                MacosLinuxVzPackageExecutionOperationV1::WheelInstallThenConsoleEntryPointHelp {
                    install_template_sha256: digest("wheel install"),
        ...
        for (operation, expected_probe) in profiles {
            let stages = stages_for_operation_v1(&operation).expect("closed console stages");
            assert_eq!(stages.len(), 3);
            assert_eq!(stages[2].stage_name(), expected_probe);
        }
    }
    ...
    #[test]
    fn empty_sdist_closure_is_validated_without_an_install_stage() {
