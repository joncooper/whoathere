...
use whoathere_artifact::{
    ArtifactEnvelope, ArtifactFormat, ArtifactManifest, Ecosystem, MemberType,
    NormalizationCompleteness, PackageIdentity, Sha256Digest, WheelMetadata,
};
...
pub enum WheelConsoleArgumentProfileV1 {
    /// Execute the console wrapper generated by the exact offline wheel install with `--help`.
    ///
    /// This is deliberately not permission to import and call the declared target directly. The
    /// generated wrapper is part of the installed artifact surface and is the behavior package
    /// users actually invoke.
    InstalledGeneratedWrapperHelp,
    /// Execute the console wrapper generated by the exact offline wheel install with no arguments.
    ///
    /// Packages commonly gate behavior on an empty argv and may return before the `--help` path.
    /// This therefore remains a separate fresh-VM scenario rather than an alias for the help probe.
    InstalledGeneratedWrapperNoArguments,
}
...
pub fn expected_wheel_scenario_kinds_v1(
    ...
    validate_wheel_manifest_contract(manifest, wheel)?;
    validate_supported_wheel_execution_contract(manifest, wheel)?;
...
fn validate_wheel_manifest_contract(
    manifest: &ArtifactManifest,
    wheel: &WheelMetadata,
) -> Result<(), ArtifactScenarioCompileErrorV1> {
    ...
    let dist_info = wheel
        ...
        .as_deref()
        .ok_or(ArtifactScenarioCompileErrorV1::InvalidManifest)?;
    let mut observed_pth_file_ids = manifest
        .members
        .iter()
        .filter(|member| {
        ...
        .collect::<Vec<_>>();
    observed_pth_file_ids.sort();
    if observed_pth_file_ids != wheel.pth_file_ids {
        return Err(ArtifactScenarioCompileErrorV1::InvalidTriggerSurface);
    }

    for target in wheel.console_entry_points.values() {
        let (module, _) = parse_console_target(target)?;
        let import_root = module.split('.').next().unwrap_or_default();
        if wheel
            .import_roots
            .binary_search_by(|candidate| candidate.as_str().cmp(import_root))
            .is_err()
        {
            return Err(ArtifactScenarioCompileErrorV1::InvalidTriggerSurface);
        }
...
fn validate_supported_wheel_execution_contract(
    manifest: &ArtifactManifest,
    wheel: &WheelMetadata,
) -> Result<(), ArtifactScenarioCompileErrorV1> {
...
fn validate_console_name(value: &str) -> Result<(), ArtifactScenarioCompileErrorV1> {
    if !supported_wheel_console_command_name_v1(value) {
        return Err(ArtifactScenarioCompileErrorV1::InvalidTriggerSurface);
    }
    Ok(())
}

/// Whether a generated wheel console command can safely occupy the measured probe venv's `bin`.
///
/// This predicate is public so downstream request/program boundaries cannot accidentally accept a
/// command name that the exact-artifact compiler rejects.
pub fn supported_wheel_console_command_name_v1(value: &str) -> bool {
    !value.is_empty()
        && value.len() <= 128
        && !matches!(value, "." | "..")
        && !value.starts_with('-')
        ...
        && !console_name_collides_with_protected_venv_tool(value)
}

/// Pip installs console wrappers into the same virtual-environment `bin` directory that contains
/// the interpreter used for every later probe. A wrapper named `python3` (or another versioned
/// Python/Pip tool name) can replace that trusted launch path and make the package control the
/// detector. Keep those names outside the supported lane until probes use an immutable launcher
/// outside the package-controlled scripts namespace.
fn console_name_collides_with_protected_venv_tool(value: &str) -> bool {
    let value = value.to_ascii_lowercase();
    ["python", "pip"].into_iter().any(|stem| {
        value == stem
            || value.strip_prefix(stem).is_some_and(|suffix| {
                !suffix.is_empty()
                    && suffix
                        .bytes()
