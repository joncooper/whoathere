...
- Vault dynamic behavior job planning and result-binding schema that ties behavior evidence to tenant, admission request, artifact, cache key, evidence profile, job id, job kind, runner identity/session, isolation proof, egress proof, configured Vault host, freshness window, canonical log digest, and audit event id.
- Fixture-safe dynamic behavior analyzer that emits sanitized deterministic evidence for clean npm lifecycle behavior, npm postinstall canary exfiltration, PyPI PEP 517 backend abuse, Python import-time abuse, DNS tunneling, HTTPS exfiltration, delayed CI activation, native extension abuse, platform-specific payloads, and direct/git/tarball dependency risk without executing arbitrary package code.
...
- Local-dev Vault dynamic behavior simulation route that exercises fixture-safe planning, behavior evidence, binding, sanitization, and fail-closed tamper cases.
- Typed admin workflow contracts for manual review, quarantine, deny, allow-after-review, break-glass, audit search, and audit export with audit-safe output.
...
cargo run --manifest-path whoathere/Cargo.toml -p whoathere-cli -- vault dev-http POST /v1/dynamic-behavior-job-simulations fixture=clean_npm_lifecycle
cargo run --manifest-path whoathere/Cargo.toml -p whoathere-cli -- vault dev-http POST /v1/dynamic-behavior-job-simulations fixture=npm_postinstall_canary_exfil
cargo run --manifest-path whoathere/Cargo.toml -p whoathere-cli -- vault dev-http POST /v1/dynamic-behavior-job-simulations fixture=pypi_pep517_canary
cargo run --manifest-path whoathere/Cargo.toml -p whoathere-cli -- vault dev-http POST /v1/dynamic-behavior-job-simulations 'fixture=dns_tunneling_canary&wrong_context=vault'
cargo run --manifest-path whoathere/Cargo.toml -p whoathere-cli -- vault dev-http POST /v1/dynamic-behavior-job-simulations 'fixture=clean_npm_lifecycle&stale=true'
