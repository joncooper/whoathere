...
public let wheelGuestStagingReceiptSchemaV1 = "whoathere.wheel_guest_staging_receipt.v1"
private let wheelGuestStagingReceiptSignatureDomainV1 = Data(
    "whoathere.wheel_guest_staging_receipt.signature.v1\0".utf8
)

public struct WheelGuestStagingReceiptObservation: Equatable, Sendable {
...
public func verifyWheelGuestStagingReceipt(
    _ receiptData: Data,
    ...
    guard !receiptData.isEmpty, receiptData.count <= maximumWheelGuestAuthBytesV1 else {
    ...
    guard let receipt = try? JSONSerialization.jsonObject(with: receiptData) as? [String: Any],
          ...
          Set(receipt.keys) == Set([
            ...
            "staged_device", "staged_inode", "artifact_file_name", "artifact_file_mode",
            "staging_directory_mode", "package_uid", "package_gid",
            "package_execution_enabled", "signature_ed25519_hex"
          ]),
          receipt["schema_version"] as? String == wheelGuestStagingReceiptSchemaV1,
          ...
          receipt["artifact_file_name"] as? String == "artifact.whl",
          ...
          let artifactLength = wheelReceiptUInt64(artifactLengthText),
          let rehashLength = wheelReceiptUInt64(rehashLengthText),
          let device = wheelReceiptUInt64(deviceText),
          let inode = wheelReceiptUInt64(inodeText),
    ...
    return WheelGuestStagingReceiptObservation(
...
private func wheelReceiptUInt64(_ value: String) -> UInt64? {
...
private func wheelReceiptSignatureData(_ value: String) -> Data? {
