...
. "$SCRIPT_DIR/wheel-supervisor-receipt-lib.sh"
...
PUBLIC_KEY_DEST="$BUNDLE_DIR/wheel-supervisor-public-key.bin"
RECEIPT_DEST="$BUNDLE_DIR/wheel-supervisor-provisioning.json"
SUPERVISOR_BINARY=${WHOATHERE_WHEEL_SUPERVISOR_BINARY:-"$WHOATHERE_ROOT/target/release/whoathere-wheel-supervisor"}
KEYGEN_BINARY=${WHOATHERE_WHEEL_SUPERVISOR_KEYGEN_BINARY:-"$WHOATHERE_ROOT/target/release/whoathere-wheel-supervisor-keygen"}
PACKAGE_USERNAME=_whoatherepkg
PACKAGE_UID=${WHOATHERE_WHEEL_PACKAGE_UID:-499}
PACKAGE_GID=${WHOATHERE_WHEEL_PACKAGE_GID:-499}
CPU_COUNT=${WHOATHERE_WHEEL_CPU_COUNT:-2}
MEMORY_MIB=${WHOATHERE_WHEEL_MEMORY_MIB:-6144}
EXPECTED_PYTHON_VERSION=${WHOATHERE_WHEEL_EXPECTED_PYTHON_VERSION:-}
EXPECTED_PIP_VERSION=${WHOATHERE_WHEEL_EXPECTED_PIP_VERSION:-}

for value in "$PACKAGE_UID" "$PACKAGE_GID" "$CPU_COUNT" "$MEMORY_MIB"; do
  case "$value" in
    ''|0|*[!0-9]*)
      echo "wheel_supervisor_package_identity_invalid=true" >&2
      exit 64
      ;;
  esac
done
...
case "$EXPECTED_PYTHON_VERSION:$EXPECTED_PIP_VERSION" in
  *[!0-9A-Za-z._:+-]*)
    echo "wheel_supervisor_runtime_version_invalid=true" >&2
    exit 64
    ;;
esac
...
if [ ! -d "$STATE_DIR" ] || [ -L "$STATE_DIR" ]; then
  preflight_reason="wheel_supervisor_state_directory_missing_or_unsafe"
elif [ ! -d "$BUNDLE_DIR" ] || [ -L "$BUNDLE_DIR" ]; then
  preflight_reason="wheel_supervisor_bundle_directory_missing_or_unsafe"
elif [ ! -f "$DISK_IMAGE" ] || [ -L "$DISK_IMAGE" ]; then
  preflight_reason="wheel_supervisor_disk_missing_or_unsafe"
elif [ ! -x "$SUPERVISOR_BINARY" ] || [ -L "$SUPERVISOR_BINARY" ]; then
  preflight_reason="wheel_supervisor_release_binary_missing"
elif [ ! -x "$KEYGEN_BINARY" ] || [ -L "$KEYGEN_BINARY" ]; then
  preflight_reason="wheel_supervisor_keygen_binary_missing"
elif [ "$(stat -f '%u' "$STATE_DIR")" -ne "$EXPECTED_OWNER_UID" ] \
  || [ $((0$(stat -f '%Lp' "$STATE_DIR") & 0022)) -ne 0 ]; then
  preflight_reason="wheel_supervisor_state_directory_owner_or_mode_unsafe"
elif [ "$(stat -f '%u' "$BUNDLE_DIR")" -ne "$EXPECTED_OWNER_UID" ] \
  || [ $((0$(stat -f '%Lp' "$BUNDLE_DIR") & 0022)) -ne 0 ]; then
  preflight_reason="wheel_supervisor_bundle_directory_owner_or_mode_unsafe"
elif [ "$(stat -f '%u' "$DISK_IMAGE")" -ne "$EXPECTED_OWNER_UID" ] \
  || [ "$(stat -f '%l' "$DISK_IMAGE")" -ne 1 ] \
  || [ $((0$(stat -f '%Lp' "$DISK_IMAGE") & 0022)) -ne 0 ]; then
  preflight_reason="wheel_supervisor_disk_owner_or_mode_unsafe"
  ...
  preflight_reason="wheel_supervisor_release_binary_owner_or_mode_unsafe"
elif [ "$(stat -f '%u' "$KEYGEN_BINARY")" -ne "$EXPECTED_OWNER_UID" ] \
  || [ "$(stat -f '%l' "$KEYGEN_BINARY")" -ne 1 ] \
  || [ $((0$(stat -f '%Lp' "$KEYGEN_BINARY") & 0022)) -ne 0 ]; then
  preflight_reason="wheel_supervisor_keygen_binary_owner_or_mode_unsafe"
elif [ -f "$RUNTIME_PID" ]; then
  RUNTIME_VALUE=$(cat "$RUNTIME_PID" 2>/dev/null || true)
  case "$RUNTIME_VALUE" in
    ''|*[!0-9]*)
      preflight_reason="wheel_supervisor_runtime_state_invalid"
      ;;
    *)
      if kill -0 "$RUNTIME_VALUE" 2>/dev/null; then
        preflight_reason="wheel_supervisor_vm_must_be_stopped"
      fi
      ;;
  esac
fi
...
if [ "$(id -u)" -ne 0 ]; then
  echo "reason_code=wheel_supervisor_provisioning_requires_root" >&2
  exit 64
fi
...
if [ -n "${SUDO_UID:-}" ] && [ "$STATE_UID" -ne "$SUDO_UID" ]; then
  echo "reason_code=wheel_supervisor_state_owner_mismatch" >&2
  exit 64
fi

BUILD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/whoathere-wheel-supervisor.XXXXXX")
chmod 0700 "$BUILD_DIR"
KEY_DIR="$BUILD_DIR/keys"
install -d -o root -g wheel -m 0700 "$KEY_DIR"
"$KEYGEN_BINARY" "$KEY_DIR"

SEED_FILE="$KEY_DIR/wheel-supervisor-ed25519.seed"
PUBLIC_KEY_FILE="$KEY_DIR/wheel-supervisor-public-key.bin"
[ "$(stat -f '%z' "$SEED_FILE")" -eq 32 ] || exit 70
[ "$(stat -f '%z' "$PUBLIC_KEY_FILE")" -eq 32 ] || exit 70

SUPERVISOR_STAGED="$BUILD_DIR/whoathere-wheel-supervisor"
...
CONFIG_FILE="$BUILD_DIR/wheel-supervisor.json"
printf '%s' "{\"package_gid\":\"$PACKAGE_GID\",\"package_uid\":\"$PACKAGE_UID\",\"package_username\":\"$PACKAGE_USERNAME\",\"schema_version\":\"whoathere.wheel_guest_supervisor_config.v1\"}" > "$CONFIG_FILE"
...
BASE_GENERATION_ID="wheel-${PUBLIC_KEY_DIGEST%${PUBLIC_KEY_DIGEST#????????????????}}"
CLONE_IMPLEMENTATION_DIGEST=$(printf '%s' 'whoathere.swift.fclonefileat.direct.v1' | shasum -a 256 | awk '{print $1}')
GUEST_PROTOCOL_DIGEST=$(printf '%s' 'whoathere.wheel_artifact_scenario.v1' | shasum -a 256 | awk '{print $1}')

PLIST_FILE="$BUILD_DIR/com.whoathere.wheel-supervisor.plist"
cat > "$PLIST_FILE" <<'EOF'
...
<dict>
  <key>Label</key>
  <string>com.whoathere.wheel-supervisor</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/libexec/whoathere-wheel-supervisor</string>
  </array>
  <key>RunAtLoad</key>
  <true/>
  <key>KeepAlive</key>
  ...
  <string>/var/log/whoathere-wheel-supervisor.out.log</string>
  <key>StandardErrorPath</key>
  <string>/var/log/whoathere-wheel-supervisor.err.log</string>
</dict>
</plist>
EOF
...
if [ -z "$ATTACHED_DISK" ] || [ -z "$DATA_MOUNT" ] || [ ! -d "$DATA_MOUNT/Library/LaunchDaemons" ]; then
  echo "reason_code=wheel_supervisor_guest_data_volume_missing" >&2
  exit 70
fi
...
if [ "$(wc -l < "$PYTHON_EXECUTABLE_LIST" | awk '{print $1}')" -ne 1 ]; then
  echo "reason_code=wheel_supervisor_measured_python_pip_missing" >&2
  exit 70
fi
...
if [ -L "$PYTHON_EXECUTABLE" ] || [ -L "$PIP_CLI" ] || [ -L "$PIP_METADATA" ]; then
  echo "reason_code=wheel_supervisor_measured_python_pip_unsafe" >&2
  exit 70
fi
if ! PYTHON_VERSION=$(
  ...
  echo "reason_code=wheel_supervisor_measured_python_version_unavailable" >&2
  exit 70
fi
if ! PIP_VERSION=$(awk -F ': ' \
  '$1 == "Version" { print $2; found += 1 } END { if (found != 1) exit 1 }' \
  "$PIP_METADATA"); then
  echo "reason_code=wheel_supervisor_measured_pip_version_unavailable" >&2
  exit 70
fi
case "$PYTHON_VERSION:$PIP_VERSION" in
  *[!0-9A-Za-z._:+-]*|:|*:)
    echo "reason_code=wheel_supervisor_measured_python_pip_version_invalid" >&2
    exit 70
    ;;
esac
if [ -n "$EXPECTED_PYTHON_VERSION" ] && [ "$PYTHON_VERSION" != "$EXPECTED_PYTHON_VERSION" ]; then
  echo "reason_code=wheel_supervisor_measured_python_version_mismatch" >&2
  exit 70
fi
if [ -n "$EXPECTED_PIP_VERSION" ] && [ "$PIP_VERSION" != "$EXPECTED_PIP_VERSION" ]; then
  echo "reason_code=wheel_supervisor_measured_pip_version_mismatch" >&2
  exit 70
fi
...
for path in \
  ...
  "$DATA_MOUNT/var/db/whoathere/wheel-staging" \
  "$DATA_MOUNT/usr/local/libexec/whoathere-wheel-supervisor" \
  "$DATA_MOUNT/Library/Application Support/WhoaThere/wheel-supervisor.json" \
  "$DATA_MOUNT/Library/Application Support/WhoaThere/wheel-supervisor-ed25519.seed" \
  "$DATA_MOUNT/Library/LaunchDaemons/com.whoathere.wheel-supervisor.plist"; do
  if [ -L "$path" ]; then
    echo "reason_code=wheel_supervisor_guest_destination_symlink_rejected" >&2
    exit 70
  fi
done

install -d -o root -g wheel -m 0755 "$DATA_MOUNT/usr/local/libexec"
install -o root -g wheel -m 0755 "$SUPERVISOR_STAGED" "$DATA_MOUNT/usr/local/libexec/whoathere-wheel-supervisor"
install -d -o root -g wheel -m 0700 "$DATA_MOUNT/Library/Application Support/WhoaThere"
install -o root -g wheel -m 0400 "$CONFIG_FILE" "$DATA_MOUNT/Library/Application Support/WhoaThere/wheel-supervisor.json"
install -o root -g wheel -m 0400 "$SEED_FILE" "$DATA_MOUNT/Library/Application Support/WhoaThere/wheel-supervisor-ed25519.seed"
install -d -o root -g wheel -m 0700 "$DATA_MOUNT/var/db/whoathere"
install -d -o root -g wheel -m 0700 "$DATA_MOUNT/var/db/whoathere/wheel-staging"
install -o root -g wheel -m 0644 "$PLIST_FILE" "$DATA_MOUNT/Library/LaunchDaemons/com.whoathere.wheel-supervisor.plist"
xattr -d com.apple.quarantine "$DATA_MOUNT/usr/local/libexec/whoathere-wheel-supervisor" >/dev/null 2>&1 || true
sync
hdiutil detach "$ATTACHED_DISK" >/dev/null
ATTACHED_DISK=""

RECEIPT_FILE="$BUILD_DIR/wheel-supervisor-provisioning.json"
...
PUBLIC_TMP="$BUNDLE_DIR/.wheel-supervisor-public-key.bin.$$"
RECEIPT_TMP="$BUNDLE_DIR/.wheel-supervisor-provisioning.json.$$"
...
echo "wheel_supervisor_provisioned=true"
