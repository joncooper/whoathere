...
static void fail(const char *message) {
    fprintf(stderr, "canonical_runtime_qualification_newc: %s\n", message);
    exit(1);
}
...
static void write_trailer(FILE *output) {
    ...
    write_header(output, 12, 0, 1, 0, sizeof(trailer));
...
int main(int argument_count, char **arguments) {
    if (argument_count != 11) {
        fprintf(
            stderr,
            "usage: canonical_runtime_qualification_newc OUTPUT INIT AGENT MODULE_MANIFEST RUNTIME_MANIFEST VIRTIO_BLK CRC16 MBCACHE JBD2 EXT4\n"
        );
        return 64;
    }
    ...
    if (output == NULL) {
        fprintf(
            stderr,
            "canonical_runtime_qualification_newc: output open failed: %s\n",
            strerror(errno)
        );
        return 1;
    }
    ...
    write_file_entry(output, 3, 0100700, "whoathere/runtime-qualification-agent", arguments[3]);
    write_file_entry(output, 4, 0100400, "whoathere/runtime-module-bundle.json", arguments[4]);
    write_file_entry(output, 5, 0100400, "whoathere/runtime-manifest.json", arguments[5]);
    write_directory_entry(output, 6, 0040700, "whoathere/modules");
    write_file_entry(output, 7, 0100400, "whoathere/modules/virtio_blk.ko", arguments[6]);
    write_file_entry(output, 8, 0100400, "whoathere/modules/crc16.ko", arguments[7]);
    write_file_entry(output, 9, 0100400, "whoathere/modules/mbcache.ko", arguments[8]);
    write_file_entry(output, 10, 0100400, "whoathere/modules/jbd2.ko", arguments[9]);
    write_file_entry(output, 11, 0100400, "whoathere/modules/ext4.ko", arguments[10]);
