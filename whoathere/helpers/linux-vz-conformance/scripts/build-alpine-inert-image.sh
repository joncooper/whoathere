...
/bin/cat "$output/whoathere-overlay.cpio" >> "$combined_initramfs_tmp"
mv -f "$combined_initramfs_tmp" "$output/whoathere-initramfs-virt"
combined_initramfs_tmp=
copy_regular_file "$output/base/boot/vmlinuz-virt" "$output/vmlinuz-virt"
...
kernel_sha256=$(shasum -a 256 "$output/vmlinuz-virt" | awk '{print $1}')
