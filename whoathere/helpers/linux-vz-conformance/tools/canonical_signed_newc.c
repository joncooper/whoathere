...
static void fail(const char *message) {
    fprintf(stderr, "canonical_signed_newc: %s\n", message);
    exit(1);
}
...
static void write_trailer(FILE *output) {
    ...
    write_header(output, 9, 0, 1, 0, sizeof(trailer));
...
int main(int argument_count, char **arguments) {
    if (argument_count != 8) {
        fprintf(
            stderr,
            "usage: canonical_signed_newc OUTPUT INIT GUEST_SIGNER GUEST_SEED VSOCK COMMON TRANSPORT\n"
        );
        return 64;
    }
    checked_file_size(arguments[2]);
    checked_file_size(arguments[3]);
    if (checked_file_size(arguments[4]) != 32) {
        fail("guest seed must be exactly 32 bytes");
    }
    checked_file_size(arguments[5]);
    checked_file_size(arguments[6]);
    checked_file_size(arguments[7]);

    FILE *output = fopen(arguments[1], "wbx");
    if (output == NULL) {
        fprintf(stderr, "canonical_signed_newc: output open failed: %s\n", strerror(errno));
        return 1;
    }
    write_file_entry(output, 1, 0100755, "init", arguments[2]);
    write_directory_entry(output, 2, 0040711, "whoathere");
    write_file_entry(output, 3, 0100700, "whoathere/guest-signer", arguments[3]);
    write_file_entry(output, 4, 0100600, "whoathere/guest-ed25519.seed", arguments[4]);
    write_directory_entry(output, 5, 0040700, "whoathere/modules");
    write_file_entry(output, 6, 0100400, "whoathere/modules/vsock.ko", arguments[5]);
    write_file_entry(
        output,
        7,
        0100400,
