...
static void fail(const char *message) {
    fprintf(stderr, "canonical_package_sensor_newc: %s\n", message);
    exit(1);
}

static void write_bytes(FILE *output, const void *bytes, size_t length) {
    if (length > 0 && fwrite(bytes, 1, length, output) != length) fail("write failed");
...
static void write_padding(FILE *output, uint64_t alignment) {
    ...
    if (remainder != 0) write_bytes(output, zeros, (size_t)(alignment - remainder));
}
...
static void write_header(
    ...
    if (length != 110) fail("header encoding failed");
...
static uint32_t checked_file_size(const char *path) {
    ...
    if (lstat(path, &metadata) != 0) fail("input stat failed");
...
static void write_file_entry(
    ...
    if (name_size > UINT32_MAX) fail("archive name exceeds newc size limit");
    write_header(output, inode, mode, 1, file_size, (uint32_t)name_size);
    write_name(output, archive_name);

    FILE *input = fopen(source_path, "rb");
    if (input == NULL) fail("input open failed");
...
static void write_trailer(FILE *output) {
    ...
    write_header(output, 5, 0, 1, 0, sizeof(trailer));
...
int main(int argument_count, char **arguments) {
    if (argument_count != 5) {
        fprintf(
            stderr,
            "usage: canonical_package_sensor_newc OUTPUT INIT PROBE PROCESS_FIXTURE\n"
        );
        return 64;
    }
    ...
    if (output == NULL) {
        fprintf(
            stderr,
            "canonical_package_sensor_newc: output open failed: %s\n",
            strerror(errno)
        );
        return 1;
    }
    write_file_entry(output, 1, 0100755, "init", arguments[2]);
    write_directory_entry(output, 2, 0040700, "whoathere");
    write_file_entry(
        output,
        3,
        0100700,
        "whoathere/package-sensor-bpf-inert-probe",
        arguments[3]
    );
    write_file_entry(
        output,
