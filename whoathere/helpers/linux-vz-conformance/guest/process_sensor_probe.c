...
#define RAW_FRAME_PAYLOAD_LENGTH 16
#define RAW_FRAME_TARGET_PORT 40553
#define LOOPBACK_ADDRESS "127.0.0.1"
#define LOOPBACK_TARGET_PORT 40552
#define PRIVATE_SOURCE_ADDRESS "10.0.0.2"
#define PRIVATE_TARGET_ADDRESS "10.0.0.1"
#define LINK_LOCAL_SOURCE_ADDRESS "169.254.100.2"
...
static int wait_until_deadline_while_child_runs(
    pid_t child,
    int *status,
    uint64_t deadline_ns
) {
...
static int read_exact_term_resistance_report(
    int descriptor,
    struct term_resistance_report *report
) {
    size_t offset = 0;
    while (offset < sizeof(*report)) {
        ssize_t length = read(descriptor, (char *)report + offset, sizeof(*report) - offset);
    ...
    return length == 0 && report->magic == TERM_RESISTANCE_REPORT_MAGIC &&
        report->process_pid > 0 ? 0 : -1;
}

static int read_exact_vm_stop_report(int descriptor, struct vm_stop_report *report) {
    size_t offset = 0;
    while (offset < sizeof(*report)) {
        ssize_t length = read(descriptor, (char *)report + offset, sizeof(*report) - offset);
        if (length < 0 && errno == EINTR) continue;
    ...
    return length == 0 && report->magic == VM_STOP_REPORT_MAGIC &&
        report->process_pid > 0 ? 0 : -1;
}
...
static int read_exact_udp_report_for(
    int descriptor,
    struct udp_report *report,
    ...
    struct in_addr source = {0};
    struct in_addr target = {0};
    return length == 0 && report->magic == report_magic &&
        report->process_pid > 0 && report->socket_inode > 0 &&
        ...
        report->payload_length == payload_length ? 0 : -1;
}

static int read_exact_udp_report(int descriptor, struct udp_report *report) {
    return read_exact_udp_report_for(
        descriptor, report, UDP_REPORT_MAGIC,
        NETWORK_SOURCE_ADDRESS, NETWORK_TARGET_ADDRESS,
        NETWORK_TARGET_PORT, UDP_PAYLOAD_LENGTH
    );
}

static int read_exact_raw_frame_report(int descriptor, struct udp_report *report) {
    return read_exact_udp_report_for(
        descriptor, report, RAW_FRAME_REPORT_MAGIC,
        NETWORK_SOURCE_ADDRESS, NETWORK_TARGET_ADDRESS,
        RAW_FRAME_TARGET_PORT, RAW_FRAME_PAYLOAD_LENGTH
    );
}

static int read_exact_dns_plaintext_report(int descriptor, struct udp_report *report) {
    return read_exact_udp_report_for(
        descriptor, report, DNS_PLAINTEXT_REPORT_MAGIC,
        DNS_SOURCE_ADDRESS, DNS_TARGET_ADDRESS,
        DNS_TARGET_PORT, DNS_PLAINTEXT_PAYLOAD_LENGTH
    );
}
...
static int read_exact_classified_report(
    int descriptor,
...
static int prepare_ipv4_sinkhole_addresses(const char *source_address, const char *target_address) {
    ...
    if (ioctl(descriptor, SIOCSIFFLAGS, &interface) != 0 ||
        set_interface_address(descriptor, SIOCSIFADDR, source_address) != 0 ||
    ...
    if (inet_pton(AF_INET, target_address, &protocol->sin_addr) != 1) goto done;
...
static int prepare_ipv4_sinkhole(void) {
    return prepare_ipv4_sinkhole_addresses(NETWORK_SOURCE_ADDRESS, NETWORK_TARGET_ADDRESS);
}

static int prepare_private_sinkhole(void) {
    return prepare_ipv4_sinkhole_addresses(PRIVATE_SOURCE_ADDRESS, PRIVATE_TARGET_ADDRESS);
}

static int prepare_link_local_sinkhole(void) {
    return prepare_ipv4_sinkhole_addresses(
        LINK_LOCAL_SOURCE_ADDRESS, LINK_LOCAL_TARGET_ADDRESS
    );
}

static int prepare_metadata_sinkhole(void) {
    return prepare_ipv4_sinkhole_addresses(
...
static int prepare_dns_sinkhole(void) {
    return prepare_ipv4_sinkhole_addresses(DNS_SOURCE_ADDRESS, DNS_TARGET_ADDRESS);
}
...
static int proc_tcp_contains_classified_sinkhole(
    const struct network_report *report,
    const char *expected_local_address,
    ...
    char expected_local[14];
    snprintf(
        expected_local, sizeof(expected_local), "%s:%04X",
        expected_local_address, report->source_port
    );
    char line[512];
    int observed = 0;
    while (fgets(line, sizeof(line), stream) != NULL) {
...
static int proc_tcp_contains_private_sinkhole(const struct network_report *report) {
    return proc_tcp_contains_classified_sinkhole(report, "0200000A", "0100000A:01BB");
}

static int proc_tcp_contains_link_local_sinkhole(const struct network_report *report) {
    return proc_tcp_contains_classified_sinkhole(report, "0264FEA9", "0164FEA9:01BB");
}

static int proc_tcp_contains_metadata_sinkhole(const struct network_report *report) {
...
static int run_process_probe(const char *fixture, const char *fixture_case) {
    ...
    struct observation kill_event = {0};
    pid_t child = -1;
    ...
    struct udp_report raw_frame_report = {0};
    ...
    uint64_t timeout_deadline_ns = 0;
    uint64_t term_signal_timestamp_ns = 0;
    int result = 70;
    const char *failure_stage = "preflight";
    int failure_errno = 0;
    ...
    int timeout_case = strcmp(fixture_case, "timeout") == 0;
    ...
    int raw_frame_attachment = strcmp(fixture_case, "raw_frame_attachment") == 0;
    ...
    int udp_activity = udp_send || raw_frame_attachment || dns_activity;
    ...
    if (timeout_case || term_resistance || escaped_session || reparented_child ||
        ...
        fds.links[4] = fds.programs[4] >= 0
            ? attach_raw_tracepoint("sys_enter", fds.programs[4]) : -1;
        if (fds.programs[4] < 0 || fds.links[4] < 0) {
            failure_stage = "teardown_kill_attach";
            failure_errno = errno;
            goto cleanup;
        }
    }
    ...
    if (network_activity) {
        fds.programs[7] = load_syscall_program(
            fds.map,
            udp_activity ? OBSERVATION_SENDTO : OBSERVATION_CONNECT,
            udp_activity ? SYS_sendto : SYS_connect
        );
        fds.links[7] = fds.programs[7] >= 0
            ? attach_raw_tracepoint("sys_enter", fds.programs[7]) : -1;
        if (fds.programs[7] < 0 || fds.links[7] < 0) {
            failure_stage = udp_activity ? "network_sendto_attach" : "network_connect_attach";
            failure_errno = errno;
            goto cleanup;
        }
    ...
    if (timeout_case) {
        uint64_t now = monotonic_ns();
        if (now == 0 || UINT64_MAX - now < 1000000000ULL) {
            failure_stage = "timeout_deadline_create";
            goto cleanup;
        }
    ...
    if (term_resistance) {
        if (read_exact_term_resistance_report(report_pipe[0], &term_resistance_report) != 0) {
            failure_stage = "term_resistance_report";
            failure_errno = errno;
            goto cleanup;
        }
        close_if_open(&report_pipe[0]);
        if (term_resistance_report.process_pid != child ||
            !proc_identity_matches(child, parent) || !cgroup_contains_pid(child)) {
            failure_stage = "term_resistance_identity";
            goto cleanup;
        }
        uint64_t now = monotonic_ns();
        if (now == 0 || UINT64_MAX - now < 1000000000ULL) {
            failure_stage = "term_resistance_deadline_create";
            goto cleanup;
        }
        timeout_deadline_ns = now + 1000000000ULL;
    }
    ...
    if (setsid_escape || escaped_session) {
        ...
        if (escaped_session) {
            if (UINT64_MAX - session_timestamp < 1000000000ULL) {
                failure_stage = "escaped_session_deadline_create";
                goto cleanup;
            }
            timeout_deadline_ns = session_timestamp + 1000000000ULL;
        }
    }
    if (credential_change) {
    ...
    if (private_address_connect) {
        if (read_exact_private_report(report_pipe[0], &private_report) != 0) {
            failure_stage = "private_report";
            failure_errno = errno;
            goto cleanup;
        }
        close_if_open(&report_pipe[0]);
        if (private_report.process_pid != child ||
            !proc_identity_matches(child, parent) ||
            !proc_has_no_supplementary_groups(child) ||
            !proc_tcp_contains_private_sinkhole(&private_report)) {
            failure_stage = "private_live_socket";
            goto cleanup;
        }
        network_timestamp = monotonic_ns();
        if (network_timestamp == 0) {
            failure_stage = "private_timestamp";
            goto cleanup;
        }
    }
    if (link_local_connect) {
        if (read_exact_link_local_report(report_pipe[0], &link_local_report) != 0) {
            failure_stage = "link_local_report";
            failure_errno = errno;
            goto cleanup;
        }
        close_if_open(&report_pipe[0]);
        if (link_local_report.process_pid != child ||
            !proc_identity_matches(child, parent) ||
            !proc_has_no_supplementary_groups(child) ||
            !proc_tcp_contains_link_local_sinkhole(&link_local_report)) {
            failure_stage = "link_local_live_socket";
            goto cleanup;
        }
        network_timestamp = monotonic_ns();
        if (network_timestamp == 0) {
            failure_stage = "link_local_timestamp";
            goto cleanup;
        }
    }
    if (metadata_address_connect) {
        if (read_exact_metadata_report(report_pipe[0], &metadata_report) != 0) {
            failure_stage = "metadata_report";
            failure_errno = errno;
            goto cleanup;
        }
        close_if_open(&report_pipe[0]);
        if (metadata_report.process_pid != child ||
            !proc_identity_matches(child, parent) ||
            !proc_has_no_supplementary_groups(child) ||
            !proc_tcp_contains_metadata_sinkhole(&metadata_report)) {
            failure_stage = "metadata_live_socket";
            goto cleanup;
        }
        network_timestamp = monotonic_ns();
        if (network_timestamp == 0) {
            failure_stage = "metadata_timestamp";
            goto cleanup;
        }
    }
    ...
    if (raw_frame_attachment) {
        if (read_exact_raw_frame_report(report_pipe[0], &raw_frame_report) != 0) {
            failure_stage = "raw_frame_report";
            failure_errno = errno;
            goto cleanup;
        }
        close_if_open(&report_pipe[0]);
        if (raw_frame_report.process_pid != child ||
            !proc_identity_matches(child, parent) ||
            !proc_has_no_supplementary_groups(child) ||
            !proc_udp_contains_ipv4_sinkhole(&raw_frame_report)) {
            failure_stage = "raw_frame_live_socket";
            goto cleanup;
        }
        network_timestamp = monotonic_ns();
        if (network_timestamp == 0) {
            failure_stage = "raw_frame_timestamp";
            goto cleanup;
        }
    }
    if (dns_plaintext) {
        if (read_exact_dns_plaintext_report(report_pipe[0], &dns_report) != 0) {
            failure_stage = "dns_plaintext_report";
            failure_errno = errno;
            goto cleanup;
        }
        close_if_open(&report_pipe[0]);
        if (dns_report.process_pid != child ||
            !proc_identity_matches(child, parent) ||
            !proc_has_no_supplementary_groups(child) ||
            !proc_udp_contains_ipv4_sinkhole(&dns_report)) {
            failure_stage = "dns_plaintext_live_socket";
            goto cleanup;
        }
        network_timestamp = monotonic_ns();
        if (network_timestamp == 0) {
            failure_stage = "dns_plaintext_timestamp";
            goto cleanup;
        }
    }
    if (dns_malformed) {
    ...
    if (timeout_case || term_resistance || escaped_session || reparented_child ||
        ...
        if (deadline_result != 0 || !proc_identity_matches(teardown_target, parent) ||
            ...
            failure_stage = deadline_result == 1
                ? "teardown_predeadline_exit" : "teardown_live_identity";
            failure_errno = errno;
            failure_detail = deadline_result;
            goto cleanup;
        }
        if (escaped_session &&
            (getsid(child) != child || getpgid(child) != child ||
             session_report.session_id != child || session_report.process_group_id != child)) {
            failure_stage = "escaped_session_deadline_identity";
            failure_errno = errno;
            goto cleanup;
        }
        if (background_listener &&
        ...
        if (timeout_case || escaped_session || reparented_child || background_listener) {
            ...
            if (kill(child, SIGTERM) != 0) {
                failure_stage = "term_resistance_term_delivery";
                failure_errno = errno;
                goto cleanup;
            }
            term_signal_timestamp_ns = monotonic_ns();
            if (term_signal_timestamp_ns == 0 ||
                UINT64_MAX - term_signal_timestamp_ns < TERM_RESISTANCE_GRACE_NS) {
                failure_stage = "term_resistance_grace_create";
                goto cleanup;
            }
            int grace_result = wait_until_deadline_while_child_runs(
                child,
                &child_status,
                term_signal_timestamp_ns + TERM_RESISTANCE_GRACE_NS
            );
            if (grace_result != 0 || !proc_identity_matches(child, parent) ||
                !cgroup_contains_pid(child)) {
                failure_stage = grace_result == 1
                    ? "term_resistance_not_proven" : "term_resistance_grace_identity";
                failure_errno = errno;
                failure_detail = grace_result;
                goto cleanup;
            }
            if (kill(child, SIGKILL) != 0 || waitpid(child, &child_status, 0) != child ||
                !WIFSIGNALED(child_status) || WTERMSIG(child_status) != SIGKILL) {
                failure_stage = "term_resistance_kill_delivery";
                failure_errno = errno;
                failure_detail = WIFSIGNALED(child_status) ? WTERMSIG(child_status) : 255;
                goto cleanup;
            }
    ...
    if ((!double_fork && !reparent && !setsid_escape && !credential_change &&
        ...
        (timeout_case &&
         (!observed_pid(&fork_event, parent) || !observed_pid(&exec_event, child) ||
          !observed_pid(&kill_event, parent) || !observed_pid(&exit_event, child) ||
          fork_event.timestamp_ns >= exec_event.timestamp_ns ||
          exec_event.timestamp_ns >= kill_event.timestamp_ns ||
          kill_event.timestamp_ns >= exit_event.timestamp_ns)) ||
        (term_resistance &&
         (!observed_pid(&fork_event, parent) || !observed_pid(&exec_event, child) ||
          kill_event.count != 2 || (pid_t)(kill_event.pid_tgid >> 32) != parent ||
          kill_event.timestamp_ns == 0 || !observed_pid(&exit_event, child) ||
          term_signal_timestamp_ns == 0 ||
          fork_event.timestamp_ns >= exec_event.timestamp_ns ||
          exec_event.timestamp_ns >= term_signal_timestamp_ns ||
          term_signal_timestamp_ns >= kill_event.timestamp_ns ||
          kill_event.timestamp_ns >= exit_event.timestamp_ns)) ||
        (escaped_session &&
         (!observed_pid(&fork_event, parent) || !observed_pid(&exec_event, child) ||
    ...
    if (term_resistance) {
        puts("WHOATHERE_SENSOR teardown_trigger=deadline");
        puts("WHOATHERE_SENSOR teardown_deadline=reached");
        puts("WHOATHERE_SENSOR teardown_term_signal=resisted");
        puts("WHOATHERE_SENSOR teardown_term_grace=reached");
        puts("WHOATHERE_SENSOR teardown_kill_signal=delivered");
        puts("WHOATHERE_SENSOR teardown_descendants=none_remaining");
        puts("WHOATHERE_SENSOR teardown_sensor=closed");
        puts("WHOATHERE_SENSOR teardown_cgroup=removed");
        puts("WHOATHERE_SENSOR teardown_terminal=timeout_with_teardown");
    ...
    if (raw_frame_attachment) {
        puts("WHOATHERE_SENSOR raw_frame_attachment=observed");
        puts("WHOATHERE_SENSOR raw_frame_payload=whoathere_raw_v1_16_bytes");
        puts("WHOATHERE_SENSOR raw_frame_target=documentation_sinkhole_192_0_2_1_40553");
    }
    if (dns_plaintext) {
        puts("WHOATHERE_SENSOR network_dns_plaintext=observed");
        puts("WHOATHERE_SENSOR network_socket_state=unconnected_bound");
    ...
    if (private_address_connect) {
        puts("WHOATHERE_SENSOR network_private_address_connect=observed");
        puts("WHOATHERE_SENSOR network_socket_state=syn_sent");
        puts("WHOATHERE_SENSOR network_destination_class=private_rfc1918");
        puts("WHOATHERE_SENSOR network_target=private_sinkhole_10_0_0_1_443");
    }
    if (link_local_connect) {
        puts("WHOATHERE_SENSOR network_link_local_connect=observed");
        puts("WHOATHERE_SENSOR network_socket_state=syn_sent");
        puts("WHOATHERE_SENSOR network_destination_class=link_local");
        puts("WHOATHERE_SENSOR network_target=link_local_sinkhole_169_254_100_1_443");
    }
    if (metadata_address_connect) {
        puts("WHOATHERE_SENSOR network_metadata_address_connect=observed");
        puts("WHOATHERE_SENSOR network_socket_state=syn_sent");
    ...
    if (escaped_session) {
        printf(
            ...
            "\",\"kind\":\"exit\",\"sequence\":\"5\",\"subject_pid\":\"%" PRIu64
            "\",\"timestamp_ns\":\"%" PRIu64 "\"}],"
            "\"evidence_truncated\":false,\"fixture_case\":\"escaped_session\","
            "\"fixture_termination_signal\":\"15\",\"heartbeat_count\":\"2\","
            "\"kill_signal_count\":\"0\",\"package_gid\":\"65534\","
            "\"package_uid\":\"65534\",\"reaped_process_count\":\"1\","
            "\"schema_version\":\"whoathere.linux_vz_teardown_evidence_payload.v1\","
            "\"sensor_healthy\":true,\"sensor_teardown_complete\":true,"
            "\"session_escape_count\":\"1\","
            "\"session_target\":\"new_session_leader_at_deadline\","
            "\"teardown_trigger\":\"deadline\",\"termination_signal_count\":\"1\"}\n",
            (uint64_t)parent,
            cgroup.count,
            (uint64_t)child,
    ...
    if (timeout_case) {
        printf(
            ...
            "\",\"timestamp_ns\":\"%" PRIu64 "\"},"
            "{\"actor_pid\":\"%" PRIu64 "\",\"cgroup_id\":\"%" PRIu64
            "\",\"kind\":\"signal_term\",\"sequence\":\"3\","
            "\"subject_pid\":\"%" PRIu64 "\",\"timestamp_ns\":\"%" PRIu64 "\"},"
            "{\"actor_pid\":\"%" PRIu64 "\",\"cgroup_id\":\"%" PRIu64
            "\",\"kind\":\"exit\",\"sequence\":\"4\",\"subject_pid\":\"%" PRIu64
            "\",\"timestamp_ns\":\"%" PRIu64 "\"}],"
            "\"evidence_truncated\":false,\"fixture_case\":\"timeout\","
            "\"fixture_termination_signal\":\"15\",\"heartbeat_count\":\"2\","
            "\"kill_signal_count\":\"0\",\"package_gid\":\"65534\","
            "\"package_uid\":\"65534\",\"reaped_process_count\":\"1\","
            "\"schema_version\":\"whoathere.linux_vz_teardown_evidence_payload.v1\","
    ...
    if (term_resistance) {
        printf(
            ...
            "\"subject_pid\":\"%" PRIu64 "\",\"timestamp_ns\":\"%" PRIu64 "\"},"
            "{\"actor_pid\":\"%" PRIu64 "\",\"cgroup_id\":\"%" PRIu64
            "\",\"kind\":\"signal_kill\",\"sequence\":\"4\","
            "\"subject_pid\":\"%" PRIu64 "\",\"timestamp_ns\":\"%" PRIu64 "\"},"
            "{\"actor_pid\":\"%" PRIu64 "\",\"cgroup_id\":\"%" PRIu64
            "\",\"kind\":\"exit\",\"sequence\":\"5\",\"subject_pid\":\"%" PRIu64
            "\",\"timestamp_ns\":\"%" PRIu64 "\"}],"
            "\"evidence_truncated\":false,\"fixture_case\":\"term_resistance\","
            "\"fixture_termination_signal\":\"9\",\"heartbeat_count\":\"2\","
            "\"kill_signal_count\":\"1\",\"package_gid\":\"65534\","
            "\"package_uid\":\"65534\",\"reaped_process_count\":\"1\","
            "\"schema_version\":\"whoathere.linux_vz_teardown_evidence_payload.v1\","
            "\"sensor_healthy\":true,\"sensor_teardown_complete\":true,"
            "\"teardown_trigger\":\"deadline\","
            "\"term_grace_limit_ns\":\"250000000\",\"term_grace_reached\":true,"
            "\"term_resistance_proven\":true,\"termination_signal_count\":\"1\"}\n",
            (uint64_t)parent,
            cgroup.count,
            (uint64_t)child,
            fork_event.timestamp_ns,
    ...
    if (network_activity) {
        ...
        const char *network_action = encrypted_dns_connect ? "encrypted_dns_connect" :
            (dns_malformed ? "dns_malformed" :
                (dns_plaintext ? "dns_query" :
                    (raw_frame_attachment ? "raw_frame_emit" :
        ...
        uint16_t network_source_port = encrypted_dns_connect ? encrypted_dns_report.source_port :
            ...
            (raw_frame_attachment ? raw_frame_report.source_port :
        ...
        uint16_t network_target_port = encrypted_dns_connect ? ENCRYPTED_DNS_TARGET_PORT :
            (dns_activity ? DNS_TARGET_PORT :
                (raw_frame_attachment ? RAW_FRAME_TARGET_PORT :
...
int main(int argument_count, char **arguments) {
    ...
    if (strcmp(arguments[2], "fork_exec_exit") == 0 ||
        ...
        strcmp(arguments[2], "raw_frame_attachment") == 0 ||
